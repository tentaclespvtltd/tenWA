...
(() => {
    function tryExpose() {
        ...
        } catch (e) { criticalLoaded = false; }

        try {
            if (!window.AuthStore.Cmd) {
        ...
        try {
            if (!window.AuthStore.OfflineMessageHandler) {
        ...
        try {
            if (!window.AuthStore.Base64Tools) {
                window.AuthStore.Base64Tools = window.require('WABase64');
            }
        } catch (e) { criticalLoaded = false; }

        // Optional modules (like pairing and registration)
        try {
            ...
            // Ignore failure for pairing code utils
        }

        try {
            if (!window.AuthStore.RegistrationUtils) {
            ...
            // Ignore failure for registration utils as they might not load if already authenticated
        }
